...
async def api_checkin(body: CheckInBody, request: Request, response: Response):
    scan_log(f"[checkin] incoming request email={body.email!r} token={body.token[:8]}...")

    if body.token != state.current_token or time.time() >= state.expires_at:
    ...
    try:
        ...
        attendee = db_get_attendee(email)
        if not attendee:
            scan_log(f"[checkin] rejected: {email} not registered")
            raise HTTPException(
                status_code=404,
                detail="This email is not registered for this event. Please use a valid attendee ticket.",
            )
        ...
        scan_log(f"[checkin] recorded ok: {email} timesScanned={times_scanned + 1} subscribers={len(state.subscribers)}")
        ...
        scan_log(f"[checkin] ERROR for {email}: {e}")
